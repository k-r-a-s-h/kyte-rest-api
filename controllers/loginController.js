...
const checkToken = async (req, res, next) => {
    ...
    try {
        const [user] = await db.execute('SELECT users.id, image, name, reset_tokens.createdAt FROM users JOIN reset_tokens ON (users.email = reset_tokens.email) WHERE token = ?', [req.body.token]);
        if (user.length === 0) {
