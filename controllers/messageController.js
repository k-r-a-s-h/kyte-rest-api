...
const getMessages = async (req, res, next) => {
    ...
    try {
        ...
        const [messages] = await db.execute('SELECT id, msg, r_id, s_id, seen, createdAt AS dts FROM messages WHERE (( r_id = ? AND s_id = ? ) OR ( r_id = ? AND s_id = ? )) ORDER BY createdAt DESC LIMIT ?, 50', [
            req.userId,
            req.body.r_id,
            req.body.r_id,
            req.userId,
