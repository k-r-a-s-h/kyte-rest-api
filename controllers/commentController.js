...
const getAllComments = async (req, res, next) => {
    ...
    try {
        if (req.relation === -1 || req.relation === 1 || req.public) {
            const [comments] = await db.execute('SELECT comments.id as id, users.id as u_id, comment, reply, comments.updatedAt, username, name, image  from comments JOIN users ON (comments.com_id = users.id) WHERE p_id = ?', [
                req.body.postId
            ]);
